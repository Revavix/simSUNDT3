...
fn set_density_ratio(metadata: &mut interfaces::Metadata, data: &mut Vec<f64>) -> () {
    if let &mut Some(ref mut imm) = &mut metadata.probe.immersion_properties {
        imm.density_ratio = data[0];
    }
}

fn set_wavespeed(metadata: &mut interfaces::Metadata, data: &mut Vec<f64>) -> () {
    if let &mut Some(ref mut imm) = &mut metadata.probe.immersion_properties {
        imm.fluid_wavespeed = data[0];
    }
}

fn set_distance(metadata: &mut interfaces::Metadata, data: &mut Vec<f64>) -> () {
    if let &mut Some(ref mut imm) = &mut metadata.probe.immersion_properties {
        imm.distance = data[0];
    }
}
